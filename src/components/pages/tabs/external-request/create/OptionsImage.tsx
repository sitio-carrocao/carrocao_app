...
export default function OptionsImage({ removeImage, viewImage, ref }: IProps) {
  ...
  }, [currentIndex, removeImage])

  const handleViewImage = useCallback(() => {
    bottomSheetModalRef.current?.dismiss()
    viewImage(currentIndex)
  }, [currentIndex, viewImage])
  ...
  return (
    <BottomSheetModal
      ...
      <BottomSheetView style={styles.bottomSheetView}>
        ...
        <View>
          <TouchableOpacity onPress={handleViewImage} style={styles.item}>
            <ImageUpscale color={theme.colors.primary.orange} />
            <Texts.SemiBold style={{ fontSize: 18 }}>Ver imagem</Texts.SemiBold>
          </TouchableOpacity>
          <View style={styles.separator} />
          <TouchableOpacity onPress={handleRemoveImage} style={styles.item}>
